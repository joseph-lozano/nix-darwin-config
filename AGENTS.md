...
- `flake.nix` pins inputs and assembles nix-darwin, Home Manager, and nix-homebrew. It also exposes the public shared-skills input and the private Pi configuration submodule to Home Manager.
...
- `vendor/pi` is the private `joseph-lozano/pi` Git submodule. Keep credentials and generated Pi state outside it. Update it explicitly with `git submodule update --remote vendor/pi` and commit the gitlink separately.
