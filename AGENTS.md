...
- Home Manager flattens the public `joseph-lozano/skills` input into `~/.agents/skills`. The public `joseph-lozano/pi` input supplies static files under `~/.pi/agent`, while `settings.json`, credentials, sessions, package state, and caches remain writable local state. Herdr owns and installs its Pi integration extension.
